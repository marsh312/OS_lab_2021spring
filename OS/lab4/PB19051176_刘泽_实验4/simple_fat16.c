...
void sector_read(FILE *fd, unsigned int secnum, void *buffer)
{
  ...
  fread(buffer, BYTES_PER_SECTOR, 1, fd); //将第secnum扇区中的内容写入buffer
}
...
char **path_split(char *pathInput, int *pathDepth_ret)
{
    ...
    *pathDepth_ret = pathDepth;
    ...
    char **temp_paths;
    temp_paths = org_path_split(pathInput);

    for(i = 0; i < pathDepth; i++){
        if(strcmp(temp_paths[i], ".") == 0){
            paths[i][0] = '.';
            for(j = 1; j < 11; j++)
                paths[i][j] = ' ';
        }
        else if(strcmp(temp_paths[i], "..") == 0){
            paths[i][0] = '.';
            paths[i][1] = '.';
            for(j = 2; j < 11; j++)
                paths[i][j] = ' ';
           ...
           for(j = 0; temp_paths[i][j]; j++)
             if(temp_paths[i][j] == '.') index = j;
           if(index == -1){//the path without expanded name
             for(j = 0; temp_paths[i][j]; j++)
	     {
                if(temp_paths[i][j] >= 'a' && temp_paths[i][j] <= 'z')
		{
                    paths[i][j] = temp_paths[i][j] - 'a' + 'A';
                }
                else 
          ...
          else
	  {
            ...
            for(; j < 8; j++)
                paths[i][j] = ' ';
            while(temp_paths[i][j + index - 7])
	    { 
                if(temp_paths[i][j + index - 7] >= 'a' && temp_paths[i][j + index - 7] <= 'z')
	                paths[i][j] = temp_paths[i][j + index -7] - 'a' + 'A';
                else 
			paths[i][j] = temp_paths[i][j + index -7];
                j++;   
                if(j >= 11)
			 break;
            }
...
WORD fat_entry_by_cluster(FAT16 *fat16_ins, WORD ClusterN)
{
  ...
  /* 代码开始 */
  WORD fat_entry_value;
  FILE *fd = fat16_ins->fd;
  fseek(fd, fat16_ins->Bpb.BPB_RsvdSecCnt * fat16_ins->Bpb.BPB_BytsPerSec + ClusterN * 2, SEEK_SET);
  fread(&(fat_entry_value), sizeof(BYTE), 2, fd);
...
{  //将簇号为ClusterN对应的簇的第一个扇区的内容读入buffer
  *FatClusEntryVal = fat_entry_by_cluster(fat16_ins, ClusterN); //设置FatClusEntryVal为该簇对应的FAT表项的值
  *FirstSectorofCluster = ((ClusterN - 2) * fat16_ins->Bpb.BPB_SecPerClus) + fat16_ins->FirstDataSector; //该簇的第一个扇区的编号

  sector_read(fat16_ins->fd, *FirstSectorofCluster, buffer); //将该簇的第一个扇区的内容写入buffer
}
...
{ //在根目录下查找path对应的目录或文件
  ...
  sector_read(fat16_ins->fd, fat16_ins->FirstRootDirSecNum, buffer);//将根目录的第一个扇区的内容写入buffer

  /* We search for the path in the root directory first */
  for (i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
  {
    memcpy(Root, &buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR], BYTES_PER_DIR); //获取第i个目录项并填充至root中
    ...
    if (is_eq && Root->DIR_Attr == ATTR_DIRECTORY && pathDepth == 1) //路径深度仅为1，说明该目录项即为path所对应的目录
    {
      return 0;
    }
    ...
    if (is_eq && Root->DIR_Attr == ATTR_DIRECTORY)//从当前匹配的子目录开始继续查找
    {
    ...
    {  
      sector_read(fat16_ins->fd, fat16_ins->FirstRootDirSecNum + RootDirCnt, buffer); //读根目录的下一个扇区
      RootDirCnt++;
    }
  }
...
int find_subdir(FAT16 *fat16_ins, DIR_ENTRY *Dir, char **paths, int pathDepth, int curDepth)
{
  ...
  ClusterN = Dir->DIR_FstClusLO;//一级子目录的首簇号
  ...
  do
  {
    ...
    DirSecCnt = 1;
    for(i = 1; i <= fat16_ins->Bpb.BPB_SecPerClus * fat16_ins->Bpb.BPB_BytsPerSec / 32; i++)
    {
      memcpy(Dir, &buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR], BYTES_PER_DIR); //获取第i个目录项并填充至Dir中
      ...
      if (i % (fat16_ins->Bpb.BPB_BytsPerSec / 32) == 0 && i != fat16_ins->Bpb.BPB_SecPerClus * fat16_ins->Bpb.BPB_BytsPerSec / 32)
      {  
        sector_read(fat16_ins->fd, FirstSectorofCluster + DirSecCnt, buffer); //读目录的下一个扇区
        DirSecCnt++;
      }
    }  
               ...
               struct fuse_file_info *fi)
{
  ...
  else
  {
    ...
    FILE *fd;
    fd = fat16_ins->fd;
    data_buffer = (BYTE*)malloc((size + offset) * sizeof(BYTE));	
    int ClusterNum = (offset + size) / (fat16_ins->Bpb.BPB_BytsPerSec * fat16_ins->Bpb.BPB_SecPerClus) + 1;//一共需读取的簇的数量
    //将需要读取的文件的相关数据写入data_buffser中，再从偏移offset处读取大小为size的数据
    for(i = 1; i <= ClusterNum; i++)
    {
      if(i == 1)
          ...
          ClusterN = fat_entry_by_cluster(fat16_ins, ClusterN);
      for(j = 1; j <= fat16_ins->Bpb.BPB_SecPerClus; j++)
      {
        SectorNum =  ((ClusterN - 2) * fat16_ins->Bpb.BPB_SecPerClus) + fat16_ins->FirstDataSector + j - 1; //扇区编号
        fseek(fd, BYTES_PER_SECTOR * SectorNum, SEEK_SET);
        size_t cur_offset = (i-1)*fat16_ins->Bpb.BPB_BytsPerSec*fat16_ins->Bpb.BPB_SecPerClus + (j-1)*fat16_ins->Bpb.BPB_BytsPerSec;
        if(offset + size >= cur_offset + BYTES_PER_SECTOR)
            fread(data_buffer + cur_offset, BYTES_PER_SECTOR, 1, fd);
        else
            fread(data_buffer + cur_offset, offset + size - cur_offset, 1, fd);  
      }        
    }
    memcpy(buffer, data_buffer + offset, size);
    free(data_buffer);	
  }      
  return size;
}
...
int dir_entry_create(FAT16 *fat16_ins,int sectorNum,int offset,char *Name, BYTE attr, WORD firstClusterNum,DWORD fileSize){
  ...
  /* File update date */
  WORD update_date = (time_ptr->tm_year - 80) * 512 + time_ptr->tm_mon * 32 + time_ptr->tm_mday;
  memcpy(entry_info+24, &(update_date), 2);
  ...
  printf("***** Name = %s*****\n", Name);
  printf("***** attr = %x****\n", attr);
  printf("***** update_time = %x****\n", update_time);	
  printf("***** update_date = %x****\n", update_date);
  printf("***** firstClusterNum = %x****\n", firstClusterNum);
  printf("***** filesize = %x *****\n", fileSize);	
  printf("*******entry_info = %x  %x %x %x , size = %ld******\n", *entry_info, *(entry_info + 11) , *(entry_info + 22), *(entry_info +24), sizeof(*entry_info)); 	
...
int fat16_mknod(const char *path, mode_t mode, dev_t devNum){
  ...
  /* 代码开始 */ 
  int pathDepth = 0;
  char *pathInput = malloc((strlen(path)+1) * sizeof(char));
  memcpy(pathInput, path, strlen(path));
  pathInput[strlen(path)] = '\0'; 	
  char **orgpaths = org_path_split(pathInput);
  
  memcpy(pathInput, path, strlen(path));
  pathInput[strlen(path)] = '\0'; 
  char **paths = path_split((char *)pathInput,&pathDepth);
  ...
  if (strcmp(prtPath, "/") == 0){
    ...
    { 
      ...
      { 
        findFlag = 1;
        offset = (i - 16 * (RootDirCnt-1) -1) * BYTES_PER_DIR; //the offset of the current unused entry
        break;
      }
      if (i % 16 == 0 && i != fat16_ins->Bpb.BPB_RootEntCnt)
      {  
  ...
  /* Else if parent directory is sub-directory */
    ...
    do{
      ...
      DirSecCnt = 1;
      for(i = 1; i <= fat16_ins->Bpb.BPB_SecPerClus * fat16_ins->Bpb.BPB_BytsPerSec / 32  && findFlag == 0; i++){
        memcpy(&Dir, sector_buffer + (((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR), BYTES_PER_DIR); //获取第i个目录项并填充至Dir中
        ...
        { 
          findFlag = 1;
          offset = (i - 16 * (DirSecCnt-1) -1) * BYTES_PER_DIR; //the offset of the current unused entry
          break;
        }
  ...
  /* Add the DIR ENTRY */
  if(findFlag == 0){
    printf("***** NOT FOUND Place To Add The Entry!****\n"); 
  }
  if (findFlag == 1){
...
int fat16_unlink(const char *path){
  ...
  if (strcmp(prtPath, "/") == 0){
    ...
    { 
      ...
      { 
        findFlag = 1;
        offset = (i - 16 * (RootDirCnt-1) -1) * BYTES_PER_DIR; //the offset of the current entry to be deleted
        break;
      }
      if (i % 16 == 0 && i != fat16_ins->Bpb.BPB_RootEntCnt)
      {  
  ...
  /* Else if parent directory is sub-directory */
    ...
    do{
      ...
      sectorNum = FirstSectorofCluster;
      DirSecCnt = 1;
      for(i = 1; i <= fat16_ins->Bpb.BPB_SecPerClus * fat16_ins->Bpb.BPB_BytsPerSec / 32  && findFlag == 0; i++){ //在簇号为ClusterN的簇下搜索
        memcpy(&Dir, sector_buffer + (((i - 1)*BYTES_PER_DIR) % BYTES_PER_SECTOR), BYTES_PER_DIR); //获取第i个目录项并填充至Dir中
	    if(strncmp(Dir.DIR_Name, paths[pathDepth-1],11) == 0)
        { 
          findFlag = 1;
          offset = (i - 16 * (DirSecCnt-1) -1) * BYTES_PER_DIR; //the offset of the current entry to be deleted
          break;
        }
  ...
  if(findFlag == 0){
    printf("***** NOT FOUND File To Be Deleted!****\n"); 
  }	
  if (findFlag == 1){
    FILE *fd = fat16_ins->fd;
    /* 代码开始 */  
    BYTE delete_tag = 0xe5;
    fseek(fd, sectorNum * fat16_ins->Bpb.BPB_BytsPerSec + offset ,SEEK_SET);
    int size_delete = fwrite(&(delete_tag), sizeof(BYTE), 1,fd);	
    /* 代码结束 */ 
    fflush(fd);
  }
  return 0;
...
void test_find_subdir() {
  ...
  for (i = 0; i < sizeof(path) / sizeof(path[0]); i++) {
    ...
    //printf("run for %d\n", i+1);	
    find_root(fat16_ins, &Dir, path[i]); 
    //printf("%s = %ld \n", Dir.DIR_Name, strlen(Dir.DIR_Name));
    assert(strncmp(Dir.DIR_Name, names[i], 11) == 0);
    assert(Dir.DIR_CrtTimeTenth == others[i][0]);
    assert(Dir.DIR_FstClusLO == others[i][1]);
    assert(Dir.DIR_FileSize == others[i][2]);
