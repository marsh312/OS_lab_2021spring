...
int split_string(char* string, char *sep, char** string_clips) {
    ...
    string_clips[0] = strtok(string, sep); //根据传入的sep对字符串进行拆解，string_clips[0]被赋值以第一个子串的首地址 
    int clip_num=0;
    ...
    do { 
        ...
        while(*tail == ' ' && tail != head)
            tail --;
        *(tail + 1) = '\0';
        string_clips[clip_num] = head; //用于清除首尾的空格 
        clip_num ++;
...
int main() {
    ...
    while (1) {
	    ...
	    if( pid == 0 )   
		    execute(argc, argv); 
            wait(NULL);   	        // wait the  external comman to execute, then continue the shell   		        
            ...
            if(pid == 0) {  
                ...
                close(pipefd[WRITE_END]);   //原先要输出在STDOUT上的数据作为写入数据送到管道写端口 
                /* 
            ...
            if(pid == 0) {  
                ...
                close(pipefd[READ_END]);  //原先要从STDIN上读入的数据变成从管道读端口读入 
            ...
            while (wait(NULL) > 0);  //子进程正常结束时wait()返回值为其pid，while继续循环,等待所有子进程结束 
        } else {    // 三个以上的命令
		    ...
		    int ret = pipe(pipefd);
                ...
                if(pid == 0) {
                    ...
                    /* TODO:除了第一条命令外，都将标准输重定向到上一个管道出口 */
                    if( i != 0 ){
